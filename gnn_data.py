...
base_path = Path(
    f"/n/holystore01/LABS/itc_lab/Lab/galaxyGNN/carol_processed_data/"
)

def read_data(data_path: Path, min_mass: float = 1e7, max_radius: float = 6000.) -> np.ndarray:
    """Load and filter galaxy catalog."""
    data = np.load(data_path)
    mask = (data['GalaxyMass'] > min_mass) & (data['GalaxyRhalf'] < max_radius)
    return data[mask]

def find_neighbors(cat_data: np.ndarray, boxsize: float, radius: float = 2000.) -> tuple:
    ...
    central_idx = df.groupby('FOFID')['GalaxyMass'].idxmax().values
...
def process_features(cat_data: np.ndarray, boxsize: float) -> tuple:
    ...
    # Separate halo mass and positions
    halo_idx = feature_names.index('HaloMass')
    pos_idx = [feature_names.index(f) for f in ['GalaxyPos_1', 'GalaxyPos_2', 'GalaxyPos_3']]
    non_halo_and_pos_idx = [i for i in range(len(feature_names)) if i not in pos_idx and i != halo_idx]
    
    halo_masses = feature_array[:, halo_idx]
    positions = feature_array[:, pos_idx]
    galaxy_features = feature_array[:, non_halo_and_pos_idx]
    galaxy_names = [name for i, name in enumerate(feature_names) if i in non_halo_and_pos_idx]
    
    return halo_masses, positions, galaxy_features, galaxy_names
...
def build_graph(idx: list, halo_masses: np.ndarray, positions: np.ndarray, 
                features: np.ndarray, feature_names: list, boxsize: float, 
                radius: float, max_galaxies: int = 100) -> Data:
    ...
    graph_names = feature_names + ['delta_x_central', 'delta_y_central', 'delta_z_central', 'distance_central']
...
def create_galaxy_graphs(data_path: Path, suite: str = 'TNG', radius: float = 2000.) -> list:
    """Main function to create galaxy graphs from catalog."""
    # Load and process data
    boxsize = BOXSIZE[suite]
    cat_data = read_data(data_path)
    ...
    # Process features
    halo_masses, positions, features, feature_names = process_features(cat_data, boxsize)
    features_means, features_stds = features.mean(axis=0), features.std(axis=0)
    halo_masses_means, halo_masses_stds = halo_masses.mean(axis=0), halo_masses.std(axis=0)
    stats = {
        'feature_names': feature_names + ['HaloMass'],
        'means': np.append(features_means, halo_masses_means),
        'stds': np.append(features_stds, halo_masses_stds),
    }
    
    # Create graphs
    graphs = []
    for i, c_idx in enumerate(central_idx):
        graph_idx = [c_idx] + neighbor_idx[i]
        graph = build_graph(
            graph_idx, halo_masses, positions, features, 
            feature_names, boxsize, radius
        )
        graphs.append(graph)
...
def process(sim, z, filepath, graph_radius,):
    start = time.time()
    ...
    graphs, stats = create_galaxy_graphs(filepath, suite=sim, radius=graph_radius)
...
if __name__ == '__main__':
    ...
    for sim, data in filenames.items():
        for z, filepath in data.items():
            process(sim, z, filepath, graph_radius)
